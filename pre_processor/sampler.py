...
def get_milliseconds(time: str) -> int:
    ...
    return int(
        time_object.minute * 60 * 1000
        + time_object.second * 1000
        + time_object.microsecond / 1000
    )
...
def sample(conf: Config) -> bool:
    ...
    input_video_path = (
        Path.cwd().joinpath(conf.input_data_path).joinpath(f"{conf.run_id}.mp4")
    )
    input_lyrics_path = (
        Path.cwd().joinpath(conf.input_data_path).joinpath(f"{conf.run_id}.csv")
    )
                ...
                if frame_ts >= lyrics_df.loc[i, "start_time"]:
                    if frame_ts <= lyrics_df.loc[i, "end_time"]:
                        if j % conf.sampling_fps == 0:
                            output_frame_path = output_folder_path.joinpath(
                                f"{frame_ts}.npy"
                            )
                            with open(str(output_frame_path), "wb") as f:
                                np.save(
                                    f, resize(curr_frame, conf.min_output_frame_dim)
                                )
                        else:
                            pass
                        j = j + 1
